...
justifiability of abortion among respondents in the United States
...
ggplot(abortion_data, aes(x = aj)) +
  ...
  labs(title = "Histogram of Abortion Attitudes",
       ...
       caption = "Fig.1: The visual shows that the distribution of attitude toward abortion is trimodal, rather than bell-shaped. This may worsen our model's ability to fit the data.")+
...
caption1 <- ggdraw() + 
  draw_label(
    "Fig.2: The visuals show that while some variables in the data set have bell-shaped distributions, others are left-skewed.",
    x = 0,
    hjust = 0, 
    size = 8
  ) +
...
caption2 <- ggdraw() + 
  draw_label("Fig.3: The visuals show that there may be a relationship between the response and each of these variables, making them likely predictors.",
    x = 0,
    hjust = 0, size = 7
  ) +
  theme(
...
ggpairs(abortion_data, 
  ...
  labs(title = "Correlation Matrix of Predictors of Interest", caption = "Fig.4: This matrix shows that the correlations between pairs of predictors are low, with the highest being 0.370.")
...
using the random seed $206$. Fitting the MLR, we obtain the following table of parameter point estimates.
...
#| echo: false
tidy(abortion_fit) %>%
  kable(digits = 3, caption = "This table displays the estimate, standard error, statistic and p.value for each parameter of the model.")
```
...
caption3 <- ggdraw() + 
  draw_label("Fig.5: 95% confidence intervals of estimates for model parameters, using points and error bars.",
    x = 0,
    hjust = 0, size = 10
  ) +
  theme(
...
predicting attitude toward the justifiability of abortion, with a R-squared
...
justifiability of abortion, we conducted a 10-fold cross-validation with seed 206,
...
ggplot(metrics %>% filter(.metric == "rsq"),
    ...
    geom_point() +
    geom_line() +
    labs(title = "R-squared", caption = "Fig.6: This plot of R-squared values demonstrates our chosen model 1 is the best of the three.")
```
...
final_table %>%
  kable(digits = 3, caption = "With the lowest AIC and BIC and highest adjusted R-squared value, our chosen model is best of the three.")
```
...
As shown in the following plot of residuals against fitted values, the linearity condition is not met as the residuals are not scattered around the horizontal red line. The constant variance condition is also not met.
...
c1 <- ggplot(data = final_fit_aug, aes(x = .fitted, y = .resid)) +
  ...
  labs(title = "Plot of residuals v. fitted values", x = "Predicted values", y = "Residuals")

c2 <- ggplot(final_fit_aug, aes(.resid)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 1) +
  stat_function(fun = dnorm, 
    args = list(mean = mean(final_fit_aug$.resid), sd = sd(final_fit_aug$.resid)), 
    lwd = 2,  color = "red"  )+
 labs(title = "Distribution of residuals")

c3 <- ggplot(final_fit_aug, aes(y = .resid, x = 1:nrow(final_fit_aug))) +
  geom_point(alpha = 0.5) +
  labs(x = "Order of data collection", y = "Residuals", title = "Plot of residuals v. order of collection")

top_row1 <- plot_grid(c1,c2, ncol = 2, nrow = 1)

caption4 <- ggdraw() + 
  draw_label("Fig.7: The linearity condition and constant variance condition is clearly not met in this plot of residuals v. fitted values.",
    x = 0,
    hjust = 0, size = 8
  ) +
  theme(
...
caption5 <- ggdraw() + 
  draw_label("Fig.8: The residuals appear to follow a normal distribution, which looks like a bell-shape, as illustrated in red.",
    x = 0,
    hjust = 0, size = 8
  ) +
  theme(
...
caption6 <- ggdraw() + 
  draw_label("Fig.9: The residuals plotted by observation order are randomly scattered, showing no structure.",
    x = 0,
    hjust = 0, size = 8
  ) +
  theme(
